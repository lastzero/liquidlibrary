...
class LaTeX_Render {
    ...
    public $_xsize_limit = 750;
    public $_ysize_limit = 500;
    public $_string_length_limit = 1000;
    ...
    public static $_latex_tags_blacklist = array(
    ...
    public function __construct ($picture_path,$picture_path_httpd,$tmp_dir) {
        ...
        $this->_tmp_filename = md5(mt_rand());
    }
    ...
    public function getFormulaURL($latex_formula) {
        ...
        $hashPath = '/'. $formula_hash[0] . '/' . $formula_hash[1] . '/' . $formula_hash[2] . '/' . $formula_hash[3];

        if(!file_exists($this->getPicturePath() . $hashPath)) {
            mkdir($this->getPicturePath() . $hashPath, 0755, true);
        }
            
        $full_path_filename = $this->getPicturePath() . $hashPath . '/' .$filename;

        if (is_file($full_path_filename)) {
            return $this->getPicturePathHTTPD() . $hashPath . '/' . $filename;
        } else {
            ...
            for ($i=0;$i<sizeof(self::$_latex_tags_blacklist);$i++) {
                if (stristr($latex_formula, self::$_latex_tags_blacklist[$i])) {
                	$this->_errorcode = 2;
                    return false;
                }
            }
     ...
     */
    public function wrap_formula($latex_formula) {
     ...
     *          cache directory
     */
    public function renderLatex($latex_formula) {
        $latex_document = $this->wrap_formula($latex_formula);

        $current_dir = getcwd();

        if(!file_exists($this->_tmp_dir)) {
            mkdir($this->_tmp_dir, 0755, true);
        }
        ...
        $status_code = exec($command, $result);
        
        // imagemagick convert ps to image and trim picture
        $command = $this->_convert_path." -antialias -density ".$this->_formula_density.
        ...
        $hashPath = '/'. $latex_hash[0] . '/' . $latex_hash[1] . '/' . $latex_hash[2] . '/' . $latex_hash[3];

        if(!file_exists($this->getPicturePath() . $hashPath)) {
            mkdir($this->getPicturePath() . $hashPath, 0755, true);
        }

        $filename = $this->getPicturePath() . $hashPath . '/' . $latex_hash . '.' . $this->_image_format;
        ...
        if (!$status_code) { 
            chdir($current_dir); $this->_errorcode = 6; return false; 
        }
